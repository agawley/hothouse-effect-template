...
void Hothouse::StartAudio(AudioHandle::InterleavingAudioCallback cb) { seed.StartAudio(cb); }

void Hothouse::StartAudio(AudioHandle::AudioCallback cb) { seed.StartAudio(cb); }

void Hothouse::ChangeAudioCallback(AudioHandle::InterleavingAudioCallback cb) {
  seed.ChangeAudioCallback(cb);
}

void Hothouse::ChangeAudioCallback(AudioHandle::AudioCallback cb) { seed.ChangeAudioCallback(cb); }
...
Hothouse::ToggleswitchPosition Hothouse::GetToggleswitchPosition(Toggleswitch tsw) {
  switch (tsw) {
    case (TOGGLESWITCH_1):
      return GetLogicalSwitchPosition(switches[SWITCH_1_UP], switches[SWITCH_1_DOWN]);
    case (TOGGLESWITCH_2):
      return GetLogicalSwitchPosition(switches[SWITCH_2_UP], switches[SWITCH_2_DOWN]);
    case (TOGGLESWITCH_3):
      return GetLogicalSwitchPosition(switches[SWITCH_3_UP], switches[SWITCH_3_DOWN]);
    default:
      seed.PrintLine(
...
Hothouse::ToggleswitchPosition Hothouse::GetLogicalSwitchPosition(Switch up, Switch down) {
  return up.Pressed() ? TOGGLESWITCH_UP
...
void Hothouse::ProcessFootswitchPresses(Switches footswitch) {
  ...
  if (is_pressed == true && footswitch_last_state[footswitch_index] == false) {
    ...
    footswitch_long_press_triggered[footswitch_index] =
        false;  // Reset long press trigger when pressed
  }

  uint32_t press_duration = now - footswitch_start_time[footswitch_index];

  if (switches[footswitch].Pressed() && press_duration >= HOLD_THRESHOLD_MS &&
      !footswitch_long_press_triggered[footswitch_index]) {
    ...
    footswitch_long_press_triggered[footswitch_index] =
        true;  // Ensure long press is only triggered once
  }
    ...
    if (!footswitch_long_press_triggered[footswitch_index]) {
      if (footswitch_press_count[footswitch_index] >= 2) {
        ...
        footswitch_press_count[footswitch_index] = 0;
                 ...
                 footswitchCallbacks->HandleNormalPress != NULL) {
